...
def Houghtrans(Arr):                     #xcos(phi) + ysin(phi) = d
        ...
        for d in range(0,inkr_d-1):             # bei 0 starten oder bei 1
            ...
            if(hRoom[s][d] >= 40 ):
                ...
                if( (len(peaks)==0) or not ([stemp,dtemp] in peaks) ):
                    ...
                    y0 = (dtemp-inkr_d)*inkr_distance  * np.sin(phi_Vektor[stemp])


                    x1 = np.round( x0 + d_max*np.sin(phi_Vektor[stemp]))
                    y1 = np.round( y0 + d_max*np.cos(phi_Vektor[stemp]))

                    x2 = np.round( x0 - d_max*np.sin(phi_Vektor[stemp]))
                    y2 = np.round( y0 - d_max*np.cos(phi_Vektor[stemp]))
    ...
    #himage.convert("RGB").save("hroom.png")
